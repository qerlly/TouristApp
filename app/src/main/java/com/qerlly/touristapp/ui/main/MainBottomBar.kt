            ...
            BottomNavigationItem(
                ...
                onClick = {
                    if(item.route == Destinations.ROADMAP_SCREEN) {
                        if (isJoined.value.isEmpty()) showErrorRoadmapToast(context)
                        else if (!locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER))
                            showErrorRoadmapLocationToast(context)
                        else kFunction()
                    } else {
...
fun showErrorRoadmapLocationToast(context: Context) {
    Toast.makeText(context, R.string.localiz, Toast.LENGTH_SHORT).show()
}
